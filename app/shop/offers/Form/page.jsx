...
//                             // Guard: only Shop can create/update offers
//                             if (!user || role !== "shop") {
//                                 alert("You do not have permission to perform this action. Shop access required.");
//                                 return;
//                             }
...
// Helper to format a Date to yyyy-mm-dd (HTML date input format)
const toInputDate = (d) => {
  if (!d) return "";
...
export default function Form() {
  ...
  // Keep a ref to current data for the periodic expiry check
  ...
  useEffect(() => {
    ...
    const fetchOffer = async () => {
      ...
      try {
        ...
        // Normalize dates to yyyy-mm-dd if they exist
        const normalized = {
          ...offer,
        ...
        // If the offer stored banner is a URL, use it as preview
        setPreview(offer?.bannerImageURL || null);

        // If the offer has applicable arrays, map them into selected arrays
  ...
  // Periodic check (every minute) to mark status expired when endDate has passed or equals today.
  // This makes sure offers become expired automatically when their end date arrives.
  useEffect(() => {
    const checkExpiry = () => {
      const cur = new Date();
      const todayStr = toInputDate(cur);
      const end = dataRef.current.endDate;
      if (end) {
        // If today's date is greater than or equal to end date -> expired
        if (todayStr >= end && dataRef.current.status !== "Expired") {
    ...
    checkExpiry();
    const idInterval = setInterval(checkExpiry, 60 * 1000); // check every minute
  ...
  const removeItem = (value, type) => {
    if (type === "shop")
      setSelectedShops(selectedShops.filter((s) => s !== value));
    if (type === "category")
      setSelectedCategories(selectedCategories.filter((c) => c !== value));
    if (type === "floor")
      setSelectedFloors(selectedFloors.filter((f) => f !== value));
  };
  ...
  // Validation shared between create & update
  const validateDatesBeforeSubmit = () => {
    ...
    // End date must be strictly after today (cannot be today or before)
  ...
  const handleSubmit = async () => {
    if (!validateDatesBeforeSubmit()) return;
    ...
    try {
      ...
      if (bannerImage) {
        const storageRef = ref(
          storage,
          `offers/${Date.now()}_${bannerImage.name}`
        );
        const snapshot = await uploadBytes(storageRef, bannerImage);
        bannerImageURL = await getDownloadURL(snapshot.ref);
      }
    ...
    <div className="flex-1 flex flex-col gap-5 rounded-xl p-6 sm:p-8 md:p-12 bg-gray-50 shadow">
      <form
        onSubmit={(e) => {
          ...
          if (authLoading) {
            alert(
              "Authentication not ready yet. Please wait a moment and try again."
            );
            return;
          }
          ...
          if (!user || role !== "shop") {
            alert(
              "You do not have permission to perform this action. Shop access required."
            );
            return;
          }
      ...
      >
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            className="border border-gray-400 rounded-lg px-3 py-2 focus:outline-none w-full"
            required
            min={getToday()} // cannot pick before today
          />
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            className="border border-gray-400 rounded-lg px-3 py-2 focus:outline-none w-full"
            required
            min={getTomorrow()} // cannot pick today or before
          />
        </div>
