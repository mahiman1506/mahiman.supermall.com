...
export default function Listview() {
    ...
    if (!data || data.length === 0) {
        return <div className="text-center py-6 text-gray-400">No floors found</div>;
    }
                ...
                <table className="w-full border-separate border-spacing-y-3">
                    <thead>
                        <tr>
                            ...
                            <td className="bg-white px-3 py-2 text-center font-semibold">Floor Name</td>
                            <td className="bg-white px-3 py-2 text-center font-semibold">Floor Number</td>
                            <td className="bg-white px-3 py-2 text-center font-semibold">Shop Capacity</td>
                            <td className="bg-white px-3 py-2 text-center font-semibold">Floor Plan</td>
                            <td className="bg-white px-3 py-2 text-center font-semibold">Status</td>
                            <td className="bg-white px-3 py-2 rounded-r-lg text-center font-semibold">Actions</td>
                        </tr>
                    </thead>
                    <tbody>
                        {data.map((floor, index) => (
                            <tr key={floor.id} className="shadow bg-white">
                                ...
                                <td className="text-center">
                                    <span
                                        className={`px-3 py-1 rounded-full text-sm font-medium ${floor.status === "Active"
                ...
                {data.map((floor, index) => (
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <span
                                className={`px-2 py-0.5 rounded-full text-[11px] font-medium ${floor.status === "Active"
                                    ? "bg-green-100 text-green-700"
                                    : "bg-red-100 text-red-700"
                                    }`}
                            >
                        ...
                        <div className="text-xs text-gray-600 grid grid-cols-2 gap-y-1 px-1">
                            <p><span className="font-medium text-gray-800">Floor No:</span> {floor.floorNumber}</p>
                            <p><span className="font-medium text-gray-800">Shops:</span> {floor.shopCapacity}</p>
                        </div>
